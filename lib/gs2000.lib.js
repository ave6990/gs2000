...
const subsetSum = (target, list) => {
    ...
    while (curList.length > 0) {
        const tempTarget = target - sum([...resList])
        curList = curList.filter( (val, index, array) => {
            if (val > tempTarget && index > 0) {
                return Math.abs(tempTarget - val) <
                    Math.abs(tempTarget - array[index - 1])
            }
            return true
        } )
        ...
        if (Math.abs(target - sum(tempList)) < 
                Math.abs(target - sum(resList)) || 
                resList.length == 0) {
            if (target - sum(tempList) < 0) {
                variants[Math.abs(target - sum(tempList))] = tempList
            } else {
    ...
    if (resList.length > 0) {
        variants[Math.abs(target - sum(resList))] = [...resList]
    }
