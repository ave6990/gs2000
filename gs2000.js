...
const readData = () => {
    ...
    data.sourceConcInUnit = parseFloat(document.getElementById('source_conc').value.replace(',', '.').replace(/\s/gi, ''))
    data.sourceConc = convert(data.sourceConcInUnit, data.sourceUnit,
        'ppm', data.component, data.diluent)
    data.targetConcInUnit = parseFloat(document.getElementById('target_conc').value.replace(',', '.').replace(/\s/gi, ''))
